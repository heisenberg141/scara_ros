...
def callback(joint_state,args):
    with open(f'sys_velocity_response.csv', 'a', newline='') as file:
        ...
        writer.writerow([v1,v1_ref,v2,v2_ref])
...
if __name__ == "__main__":
    ...
    with open(f'sys_velocity_response.csv', 'w', newline='') as file:
        writer=csv.writer(file)
        writer.writerow(["v1","v1_ref","v2","v2_ref"])
    rospy.sleep(5)
    q1=2*3.14/3
    ...
    sub=rospy.Subscriber('/scara_robot/joint_states', JointState, callback,(start_time,v1_publisher,v2_publisher,v1_ref,v2_ref))
    rospy.spin()    
