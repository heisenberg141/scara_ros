...
def callback(joint_state,args):
    with open(f'sys_response.csv', 'a', newline='') as file:
        ...
        writer.writerow([q1,q1_ref,q2,q2_ref])  
    
    

if __name__ == "__main__":
    with open(f'sys_response.csv', 'w', newline='') as file:
        writer=csv.writer(file)
        writer.writerow(["q1","q1_ref","q2","q2_ref"])

    print("Data_collection node startted")
    
    rospy.init_node('data_collection')
    start_time=time.time()
    sub=rospy.Subscriber('/scara_robot/joint_states', JointState, callback,(start_time, ))
    rospy.spin()    
